...
#include <robot_auto_reconstruction/ReturnJointStates.h>
...
bool Arm::updateJointStatePos(){
    robot_auto_reconstruction::ReturnJointStates req;
...
bool Arm::tuck(){
    ...
    if (armside_str == "r"){
        double tuck_pos[] = {-0.4,1.0,0.0,-2.05,0.0,-0.1,0.0, -0.023593,1.10728,-1.5566882,-2.124408, -1.4175, -1.8417, 0.21436};
        tuck_pos_vec.insert(tuck_pos_vec.begin(),tuck_pos, tuck_pos+14);
    }else{
        double tuck_pos[] = { 0.4,1.0,0.0,-2.05,0.0,-0.1,0.0, 0.06024,1.248526,1.789070,-1.683386,-1.7343417,-0.0962141,-0.0864407};
        tuck_pos_vec.insert(tuck_pos_vec.begin(),tuck_pos, tuck_pos+14);
    }
...
bool Arm::stretch(){
    ...
    std::vector<double> tuck_pos_vec;
    if (armside_str == "r"){
        //double tuck_pos[] = {-1.488, -0.002, -1.557, -1.693, -1.629, -1.488, -4.618};
        //double tuck_pos[] = {-1.572, 0.004, -0.008, -1.523, -0.091, -0.096, -4.784};
        double tuck_pos[] = {-1.609, 0.046, -1.557, -1.538, -0.009, -0.102, -4.850};
        tuck_pos_vec.insert(tuck_pos_vec.begin(),tuck_pos, tuck_pos+7);
    }else{
        //double tuck_pos[] = { 1.573, 0.046, 1.482, -1.482, 1.570, -1.724, -1.672};
        //double tuck_pos[] = {1.527, -0.010, 0.012, -1.516, 6.367, -0.097, -4.844};
        double tuck_pos[] = {1.613, 0.063, 1.532, -1.536, -0.141, -0.094, -7.788};
        tuck_pos_vec.insert(tuck_pos_vec.begin(),tuck_pos, tuck_pos+7);
    }
    if (!isAtPos(tuck_pos_vec)){
        return goToJointPos(tuck_pos_vec);
...
bool Arm::stretch(double pos_arm[]){
    ROS_INFO("stretching arm %s", armside_str.c_str());
    std::vector<double> tuck_pos_vec;

    tuck_pos_vec.insert(tuck_pos_vec.begin(),pos_arm, pos_arm+7);
